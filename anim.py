...
class BetterBakeAction(Operator):
    """Bake object/pose loc/scale/rotation animation to a new action"""
    bl_idname = "nla.better_bake"
    bl_label = "Better Bake Action"
    ...
    def execute(self, context):
        ...
        action = bake_action(self.frame_start,
                                        self.frame_end,
    ...
    if do_pose:
        for name, pbone in obj.pose.bones.items():
            ...
            for (f, matrix) in zip(frame_range, pose_info):
                ...
                if not all([b for b in pbone.lock_location]):
